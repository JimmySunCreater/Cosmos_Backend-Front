...
本仓库是一个用于处理Cosmos视频生成任务的API服务，支持智能队列管理、GPU资源管理、调用大模型实现prompt自动翻译和DynamoDB集成。
...
本文作者：孙健-亚马逊云科技解决方案架构师，10+ 年汽车行业从业经验，从事汽车电子电器分析、自动驾驶解决方案相关领域，对自动驾驶、软件定义汽车等云架构设计及AI应用有丰富经验。
